...
4. Finish the remaining clothes workflow: issue flow, CSV/XLSX exports, and yearly statistics beyond the now-native order headers/items flow.
5. Add real remote API wiring, notifications, background work, QA, and release hardening.

## Current count
**5 major implementation steps remain** after landing the local driver-account handoff, native vehicle-report PDF export, admin-side settings/report persistence, and the first clothes sizes/orders/items slices.
